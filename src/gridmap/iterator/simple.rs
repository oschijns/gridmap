    ...
    A: Cell,
{
    ...
    pub fn raw_iter(&self) -> Iter<'_, A, D, S, Ic> {
        Iter {
    ...
    /// Create an iterator over all the cells of the chunks of the GridMap
    pub fn raw_iter_mut(&mut self) -> IterMut<'_, A, D, S, Ic> {
        IterMut {
    ...
    Dim<[Ix; D]>: Dimension,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            // Get the next cell from the current chunk
            if let Some(cell) = self.cells.as_mut().and_then(|cells| cells.next()) {
                return Some(cell);
            }
